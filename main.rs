...
fn main() {
    ...
    loop{
        ...
        let ra = bitpack::getu(instruction as u64, 3, 6).unwrap() as u8;
        let rb = bitpack::getu(instruction as u64, 3, 3).unwrap() as u8;
        let rc = bitpack::getu(instruction as u64, 3, 0).unwrap() as u8;
        let op = bitpack::getu(instruction as u64, 4, 28).unwrap() as u8;
        ...
        match op {
            ...
            13 => {
                let rl = bitpack::getu(instruction as u64, 3, 25).unwrap() as u8;
                let vl = bitpack::getu(instruction as u64, 25, 0).unwrap() as u32;
